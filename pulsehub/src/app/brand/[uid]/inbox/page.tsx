...
import { 
  ...
  Star, Send, MoreVertical, Instagram, Twitter, Linkedin
} from 'lucide-react';
  ...
  useEffect(() => {
    const load = async () => {
      ...
      for (const [convId, msg] of latestByConv) {
        ...
        let name = 'User';
        if (otherUserId) {
          const { data: u } = await supabase.from('users').select('display_name').eq('id', otherUserId).single();
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      ...
      setMessages((data || []).map((m: any) => ({
        id: m.id,
        sender: m.sender_id === uid ? 'user' : 'other',
        content: m.content,
          ...
          {filtered.map((conv) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-secondary-500">{conv.handle} · {conv.platform}</p>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-secondary-500">
                    {conversations.find(c => c.id === selectedId)?.handle}
                  </p>
                </div>
              </div>
