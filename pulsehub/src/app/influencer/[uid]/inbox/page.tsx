'use client';

import { useEffect, useState } from 'react';
import { useParams } from 'next/navigation';
import { Card } from '@/components/ui/Card';
import { Button } from '@/components/ui/Button';
import { 
  Search, Mail, User, Clock, CheckCircle,
  Star, Send, MoreVertical, Building
} from 'lucide-react';
import { getSupabase } from '@/lib/supabase/client';

interface ConversationItem {
  id: string;
  participantName: string;
  participantAvatar: string;
  company: string;
  preview: string;
  time: string;
  unread: boolean;
  starred: boolean;
  budget?: string;
}

interface MessageDetail {
  id: string;
  sender: 'user' | 'brand';
  content: string;
  time: string;
}

export default function InfluencerInboxPage() {
  const params = useParams();
  const uid = params.uid as string;

  const [conversations, setConversations] = useState<ConversationItem[]>([]);
  const [messages, setMessages] = useState<MessageDetail[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [newMessage, setNewMessage] = useState('');
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const load = async () => {
      const supabase = getSupabase();
      const { data: cps } = await supabase
        .from('conversation_participants')
        .select('conversation_id')
        .eq('user_id', uid);

      if (!cps || cps.length === 0) {
        setLoading(false);
        return;
      }

      const ids = cps.map((c: any) => c.conversation_id);
      const { data: msgs } = await supabase
        .from('messages')
        .select('*')
        .in('conversation_id', ids)
        .order('created_at', { ascending: false });

      const latestByConv = new Map<string, any>();
      for (const m of msgs || []) {
        if (!latestByConv.has(m.conversation_id)) {
          latestByConv.set(m.conversation_id, m);
        }
      }

      const convList: ConversationItem[] = [];
      for (const [convId, msg] of latestByConv) {
        const { data: participants } = await supabase
          .from('conversation_participants')
          .select('user_id')
          .eq('conversation_id', convId)
          .neq('user_id', uid);

        const otherUserId = participants?.[0]?.user_id;
        let name = 'Brand';
        let company = '';
        if (otherUserId) {
          const { data: u } = await supabase.from('users').select('display_name, company_name').eq('id', otherUserId).single();
          name = u?.display_name || 'Brand';
          company = u?.company_name || '';
        }

        convList.push({
          id: convId,
          participantName: name,
          participantAvatar: name.charAt(0),
          company,
          preview: msg.content.substring(0, 80),
          time: new Date(msg.created_at).toLocaleDateString(),
          unread: !msg.read,
          starred: false,
        });
      }

      setConversations(convList);
      setLoading(false);
    };
    load();
  }, [uid]);

  useEffect(() => {
    if (!selectedId) return;
    const loadMessages = async () => {
      const supabase = getSupabase();
      const { data } = await supabase
        .from('messages')
        .select('*')
        .eq('conversation_id', selectedId)
        .order('created_at', { ascending: true });

      setMessages((data || []).map((m: any) => ({
        id: m.id,
        sender: m.sender_id === uid ? 'user' : 'brand',
        content: m.content,
        time: new Date(m.created_at).toLocaleString(),
      })));
    };
    loadMessages();
  }, [selectedId, uid]);

  const handleSend = async () => {
    if (!newMessage.trim() || !selectedId) return;
    const supabase = getSupabase();
    await supabase.from('messages').insert({
      conversation_id: selectedId,
      sender_id: uid,
      content: newMessage,
    });
    setNewMessage('');
    const { data } = await supabase
      .from('messages')
      .select('*')
      .eq('conversation_id', selectedId)
      .order('created_at', { ascending: true });
    setMessages((data || []).map((m: any) => ({
      id: m.id,
      sender: m.sender_id === uid ? 'user' : 'brand',
      content: m.content,
      time: new Date(m.created_at).toLocaleString(),
    })));
  };

  const filtered = conversations.filter(c =>
    c.participantName.toLowerCase().includes(searchQuery.toLowerCase()) ||
    c.preview.toLowerCase().includes(searchQuery.toLowerCase())
  );

  if (loading) {
    return <div className="flex items-center justify-center h-64 text-secondary-500">Loading inbox...</div>;
  }

  return (
    <div className="flex h-[calc(100vh-8rem)] -m-8">
      {/* Sidebar */}
      <div className="w-96 bg-white border-r border-secondary-200 flex flex-col">
        <div className="p-4 border-b border-secondary-100">
          <h1 className="text-xl font-bold text-secondary-900 mb-4">Inbox</h1>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-secondary-400" />
            <input
              type="text"
              placeholder="Search messages..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-4 py-2 bg-secondary-50 border border-secondary-200 rounded-lg text-sm focus:border-primary-500 outline-none"
            />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto">
          {filtered.map((conv) => (
            <button
              key={conv.id}
              onClick={() => setSelectedId(conv.id)}
              className={`w-full p-4 flex items-start space-x-3 border-b border-secondary-100 hover:bg-secondary-50 transition text-left ${
                selectedId === conv.id ? 'bg-primary-50' : ''
              }`}
            >
              <div className="w-10 h-10 rounded-full bg-gradient-to-r from-blue-500 to-primary-500 flex items-center justify-center text-white font-bold flex-shrink-0">
                {conv.participantAvatar}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold text-secondary-900 text-sm truncate">{conv.participantName}</h3>
                  <span className="text-xs text-secondary-500 flex-shrink-0">{conv.time}</span>
                </div>
                {conv.company && <p className="text-xs text-secondary-500">{conv.company}</p>}
                <p className="text-sm text-secondary-600 mt-1 truncate">{conv.preview}</p>
              </div>
              {conv.unread && <div className="w-2 h-2 bg-primary-600 rounded-full flex-shrink-0 mt-2" />}
            </button>
          ))}
          {filtered.length === 0 && (
            <div className="p-8 text-center text-secondary-500">No conversations yet.</div>
          )}
        </div>
      </div>

      {/* Message Panel */}
      <div className="flex-1 flex flex-col bg-white">
        {selectedId ? (
          <>
            <div className="p-4 border-b border-secondary-100 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-r from-blue-500 to-primary-500 flex items-center justify-center text-white font-bold">
                  {conversations.find(c => c.id === selectedId)?.participantAvatar || 'U'}
                </div>
                <div>
                  <h3 className="font-semibold text-secondary-900">
                    {conversations.find(c => c.id === selectedId)?.participantName || 'User'}
                  </h3>
                  <p className="text-xs text-secondary-500">
                    <Building className="w-3 h-3 inline mr-1" />
                    {conversations.find(c => c.id === selectedId)?.company || 'Brand'}
                  </p>
                </div>
              </div>
              <button className="p-2 hover:bg-secondary-100 rounded-lg">
                <MoreVertical className="w-5 h-5 text-secondary-600" />
              </button>
            </div>

            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] p-3 rounded-lg ${
                    msg.sender === 'user'
                      ? 'bg-gradient-to-r from-primary-600 to-primary-700 text-white rounded-br-sm'
                      : 'bg-secondary-100 text-secondary-900 rounded-bl-sm'
                  }`}>
                    <p className="text-sm">{msg.content}</p>
                    <p className={`text-xs mt-1 ${msg.sender === 'user' ? 'text-primary-200' : 'text-secondary-500'}`}>
                      {msg.time}
                    </p>
                  </div>
                </div>
              ))}
              {messages.length === 0 && (
                <div className="flex items-center justify-center h-full text-secondary-500">
                  No messages yet. Start the conversation!
                </div>
              )}
            </div>

            <div className="p-4 border-t border-secondary-100">
              <div className="flex items-center space-x-3">
                <input
                  type="text"
                  placeholder="Type your message..."
                  value={newMessage}
                  onChange={(e) => setNewMessage(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && handleSend()}
                  className="flex-1 px-4 py-3 bg-secondary-50 border border-secondary-200 rounded-lg text-sm focus:border-primary-500 outline-none"
                />
                <Button onClick={handleSend} disabled={!newMessage.trim()}>
                  <Send className="w-4 h-4" />
                </Button>
              </div>
            </div>
          </>
        ) : (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <Mail className="w-16 h-16 text-secondary-300 mx-auto mb-4" />
              <h3 className="text-xl font-semibold text-secondary-900 mb-2">Select a conversation</h3>
              <p className="text-secondary-600">Choose a conversation from the left to start messaging</p>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
